...
2. АРІ Эндпоинты (с использованием Resource-контроллеров и Form Request Validation):
﻿﻿Товары (Products):
﻿﻿GET /api/products — список всех товаров с пагинацией (по 10-15 штук).
 Должен включать информацию о категории.
...
4. Валидация:
• При создании/обновлении товара проверять: пате (обязательно, строка), price (обязательно, число, больше 0), category_id (существует в таблице категорий).
...
﻿﻿Фронтенд:
﻿﻿Использование композаблов (composables) для выноса логики работы с АРІ
